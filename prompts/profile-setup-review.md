다음 3개의 참조 문서를 모두 읽은 후, src/app/home/profile-setup/ 폴더와 관련 API의 전체 검수 및 코드 수정을 수행해줘.
...
⚠️ 최우선 경고 — 기존 사용자 데이터 보호 (반드시 읽고 지켜줘)
이 페이지는 과거 수정 시 2번이나 기존 사용자가 "프로필 미완료"로 잘못 분류되어 롤백한 이력이 있다.
아래 규칙을 절대적으로 지켜줘:

1. profileSetupDone 플래그를 false로 바꾸거나, 이 값을 리셋하는 코드를 절대 작성하지 마.
2. 기존 AWS DynamoDB에 이미 profileSetupDone: true인 사용자가 있다. 이 사용자들이 새 기기나 캐시 초기화 후에도 정상적으로 인식되어야 한다.
3. 새로운 가드/리다이렉트를 추가하려면 반드시 아래 두 시나리오를 모두 통과하는지 확인해줘:
   - 시나리오 A: 완전 신규 사용자 (처음 가입)
   - 시나리오 B: 기존 사용자 + 빈 로컬스토리지 (새 기기/캐시 클리어)
4. localStorage에만 의존하는 가드를 만들지 마. 반드시 AWS 데이터 hydration 이후에 판단해야 한다.
...
HealEcho_Structural_Convergence_Rules.md의 Harness Rules(N, S, D, A, DF, M, I 카테고리)를 profile-setup 파일들에 적용해줘.
- 각 룰별 PASS/FAIL 판정
- FAIL 항목은 위반 위치(파일명:라인)를 표시하고, 바로 코드를 수정해줘
- 단, profileSetupDone 판단 로직과 관련된 코드는 구조만 개선하고 동작을 변경하지 마
...
1. 구조 & 코드 품질
2. 보안
3. API 설계 (src/app/api/user/profile/route.ts 포함)
4. 데이터베이스
5. 에러 핸들링 & 로깅
6. 성능
7. 반응형 디자인 & 접근성
...
- 특히 "보안" 카테고리에서 profileSetupDone 판단 흐름이 안전한지 집중 검토해줘
