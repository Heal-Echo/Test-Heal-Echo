...
function parseRange(r?: string): number {
  if (!r) return 7;
  if (r.endsWith('d')) return parseInt(r);
  if (r.endsWith('w')) return parseInt(r) * 7;
  if (r.endsWith('m')) return parseInt(r) * 30;
  return 7;
}
...
export const handler = async (event: any) => {
  ...
  let progressBuckets: Record<string, number> = {};
  let totalWatch = 0;

  for (const it of items) {
    const et = it.eventType;
    if (et === 'impression') impressions++;
    if (et === 'play') plays++;
    if (et === 'pause') pauses++;
    if (et === 'ended') ended++;
