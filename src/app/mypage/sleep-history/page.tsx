...
export default function SleepHistoryPage() {
  ...
  // 툴팁 hover 상태
  const [qualityHover, setQualityHover] = useState<{ x: number; y: number; label: string } | null>(
    null
  );
  const [patternHover, setPatternHover] = useState<{ x: number; y: number; label: string } | null>(
    null
  );
  const [habitHover, setHabitHover] = useState<{ x: number; y: number; label: string } | null>(
    null
  );
  const [durationHover, setDurationHover] = useState<{
    x: number;
    y: number;
    label: string;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (res.ok) {
          ...
          const filtered = allItems.filter((it: SleepLogItem) => it.sleepTime && it.wakeTime);
    ...
    (async () => {
      ...
      if (!found) {
        try {
          const logRes = await fetch("/api/user/sleep-log", {
            method: "GET",
            cache: "no-store",
            headers: authHeaders,
          });
  ...
  const patternDays = useMemo((): DayData[] => {
    ...
    for (let d = 1; d <= totalDays; d++) {
      ...
      const napHours =
        napStartH !== null && napEndH !== null && napEndH > napStartH
          ? Math.round((napEndH - napStartH) * 10) / 10
          : 0;
      result.push({
        day: d,
        bedtimeDay,
        ...
        wakeCount: log.wakeCount ?? 0,
        bedtimeRaw: log.sleepTime,
        wakeTimeRaw: log.wakeTime,
        hadNap,
        napStart: napStartH,
        napEnd: napEndH,
        napHours,
        ...
        {loading ? (
          ...
          <>
                    ...
                    <div className={styles.chartBox}>
                      ...
                      >
                          ...
                          return (
                            <g key={`y-${v}`}>
                              <line
                                ...
                                x2={padL + innerW}
                                y2={y}
                                stroke={v === 0 ? "rgba(107,114,128,0.3)" : "rgba(229,231,235,0.8)"}
                        ...
                        {chartInfo.monthlyData.map((r, i) => {
                          ...
                          const isLatest =
                            i === chartInfo.monthlyData.length - 1 ||
                          ...
                          (() => {
                            ...
                            return (
                              <g>
                                ...
                                />
                                <polygon
                                  points={`${qualityHover.x - 4},${qualityHover.y - 14} ${qualityHover.x + 4},${qualityHover.y - 14} ${qualityHover.x},${qualityHover.y - 9}`}
                                  fill="rgba(55,65,81,0.92)"
                                />
                                <text
                                  x={tx}
                        ...
                        {[...psqiResults].reverse().map((r) => {
                          ...
                          return (
                            <tr key={r.testDate} className={styles.tr}>
                              ...
                              <td className={styles.tdScore} style={{ color: q.color }}>
                                {toScore10(r.total)}
                              </td>
                              <td className={styles.td}>
                                <span className={styles.tableGrade} style={{ background: q.color }}>
                                  {q.grade}
                                </span>
                                <span className={styles.tableLabel} style={{ color: q.color }}>
                                  {q.label}
                                </span>
                              </td>
                            </tr>
            ...
            {(() => {
              ...
              const toY = (hour: number) =>
                spPadT + ((patternYRange.max - hour) / range) * spInnerH;
              const toX = (day: number) => spPadL + ((day - 1) / (totalDays - 1)) * spInnerW;

              // Y축 눈금: 2시간 단위
              const spYTicks: number[] = [];
              for (
                let t = Math.ceil(patternYRange.min);
                t <= Math.floor(patternYRange.max);
                t += 2
              ) {
                spYTicks.push(t);
              }
                ...
                <section className={styles.chartSection}>
                  ...
                  <div className={styles.monthNav}>
                    ...
                    <span className={styles.monthNavLabel}>
                      {patternYear}년 {patternMonth + 1}월
                    </span>
                    <button
                      className={styles.monthNavBtn}
                      onClick={goPatternNext}
                  ...
                  {patternDays.length === 0 ? (
                    ...
                    <>
                      <div className={styles.chartBox}>
                        <svg
                          viewBox={`0 0 ${spW} ${spH}`}
                          className={styles.sleepChart}
                          style={hasAnyNap ? { aspectRatio: `${spW} / ${spH}` } : undefined}
                          preserveAspectRatio="xMidYMid meet"
                        >
                          {/* 배경 */}
                          <rect
                                ...
                                return (
                                  <g key={`nap-marker-${dd.day}`}>
                                    ...
                                    <circle
                                      ...
                                      fill="transparent"
                                      onMouseEnter={() =>
                                        setNapHover({ x, y: markerY, label: napLabel })
                                      }
                          ...
                          {napHover &&
                            (() => {
                              ...
                              return (
                                <g>
                                  <polygon
                                    points={`${napHover.x - 4},${tipTop} ${napHover.x + 4},${tipTop} ${napHover.x},${tipTop - 5}`}
                                    fill="rgba(55,65,81,0.92)"
                                  />
                                  <rect
                                    x={tx - tw / 2}
                          ...
                          {segments.map((seg, si) => {
                            ...
                            return (
                              <g key={`seg-${si}`}>
                                ...
                                <polyline
                                  points={seg.wakePts.map((p) => `${p.x},${p.y}`).join(" ")}
                                  fill="none"
                                  stroke="#f59e0b"
                                  strokeWidth="1.5"
                                  strokeLinecap="round"
                                ...
                                />
                                <polyline
                                  points={seg.bedPts.map((p) => `${p.x},${p.y}`).join(" ")}
                                  fill="none"
                                  stroke="#6366f1"
                                  strokeWidth="1.5"
                                  strokeLinecap="round"
                            ...
                            return patternDays.map((dd) => {
                              ...
                              return (
                                <g
                                  key={`spt-${dd.day}`}
                                  onMouseEnter={() =>
                                    setPatternHover({ x: wakeX, y: wakeY, label: dateLabel })
                                  }
                            ...
                            (() => {
                              ...
                              return (
                                <g>
                                  ...
                                  />
                                  <polygon
                                    points={`${patternHover.x - 4},${patternHover.y - 10} ${patternHover.x + 4},${patternHover.y - 10} ${patternHover.x},${patternHover.y - 5}`}
                                    fill="rgba(55,65,81,0.92)"
                                  />
                                  <text
                                    x={tx}
                      ...
                      <div
                        ...
                        style={{ marginTop: 6, paddingTop: 5, marginBottom: 0 }}
                      >
                      ...
                      {(() => {
                        ...
                        const toDurX = (day: number) =>
                          durPadL + ((day - 1) / (totalDays - 1)) * durInnerW;
                            ...
                            >
                                ...
                                return (
                                  <g key={`dy-${t}`}>
                                    <line
                                      x1={durPadL}
                                      y1={y}
                                      x2={durPadL + durInnerW}
                                      y2={y}
                                      stroke={
                                        t === 0 ? "rgba(107,114,128,0.3)" : "rgba(229,231,235,0.7)"
                                      }
                              ...
                              {durationHover &&
                                (() => {
                                  ...
                                  const tx = Math.max(
                                    tw / 2,
                                    Math.min(durationHover.x, durW - tw / 2)
                                  );
                                  return (
                                    <g>
                                      <rect
                                        x={tx - tw / 2}
                                        y={durationHover.y - 32}
                                        width={tw}
                                        height={18}
                                        rx={4}
                                        fill="rgba(55,65,81,0.92)"
                                      />
                                      <polygon
                                        points={`${durationHover.x - 4},${durationHover.y - 14} ${durationHover.x + 4},${durationHover.y - 14} ${durationHover.x},${durationHover.y - 9}`}
                                        fill="rgba(55,65,81,0.92)"
                                      />
                                      <text
                                        x={tx}
                                        y={durationHover.y - 20}
                                        textAnchor="middle"
                                        fontSize="9"
                                        fontWeight="600"
                                        fill="#ffffff"
                      ...
                      <div className={styles.legend} style={{ marginTop: 4, paddingTop: 4 }}>
                        <span className={styles.legendItem}>
                          <span className={styles.legendDot} style={{ background: "#10b981" }} />총
                          수면시간
                        </span>
                      </div>
                    </>
                    ...
                    <div className={styles.tableWrap}>
                      <table className={styles.table}>
                        <thead>
                          <tr>
                            <th className={styles.th}>날짜</th>
                            <th className={styles.th} style={{ color: "#6366f1" }}>
                              취침
                            </th>
                            <th className={styles.th} style={{ color: "#f59e0b" }}>
                              기상
                            </th>
                        ...
                        <tbody>
                          {patternDays.map((dd) => (
                            <tr key={dd.day} className={styles.tr}>
                              ...
                              <td className={styles.tdScore} style={{ color: "#6366f1" }}>
                                {dd.sleepHours}h
                              </td>
                              <td className={styles.td}>{dd.wakeCount}회</td>
                              <td
                                className={styles.td}
                                style={{ color: dd.napHours > 0 ? "#38bdf8" : undefined }}
                              >
                              ...
                              {sortedDays.map((d) => {
                                ...
                                return (
                                  <g
                                    key={`hdot-${idx}-${d}`}
                                    onMouseEnter={() =>
                                      setHabitHover({ x: cx, y: cy, label: dateLabel })
                                    }
                          ...
                          (() => {
                            ...
                            return (
                              <g>
                                ...
                                />
                                <polygon
                                  points={`${habitHover.x - 4},${ty + 18} ${habitHover.x + 4},${ty + 18} ${habitHover.x},${ty + 23}`}
                                  fill="rgba(55,65,81,0.92)"
                                />
                                <text
                                  x={tx}
