...
export async function GET(request: NextRequest) {
  ...
  if (error) {
    return NextResponse.redirect(
      new URL("/public/login?naver_error=cancelled", origin)
    );
  }
  ...
  if (!code || !state) {
    return NextResponse.redirect(
      new URL("/public/login?naver_error=no_code", origin)
    );
  }
  ...
  if (stateProvider !== "naver") {
    return NextResponse.redirect(
      new URL("/public/login?naver_error=invalid_state", origin)
    );
  }
  ...
  try {
    ...
    // 이메일 필수 확인
    if (!realEmail) {
      throw new Error("네이버 계정에서 이메일을 받지 못했습니다. 네이버 로그인 시 이메일 제공에 동의해주세요.");
    }
