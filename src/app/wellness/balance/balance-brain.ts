...
import type { UserSubscription } from "@/types/subscription";
  ...
  export function calculateBalanceState(
    ...
    const weeksSorted = [...weeklyVideos].sort(
    ...
    const intermediate = weeksSorted.map(
      (week): {
        ...
        // 오픈된 이후의 모든 시청 기록
        const playsFromOpen = allEvents.filter(
          (t) => t.getTime() >= openAt.getTime()
        );
  
        // 오픈일 ~ 7일 동안의 시청 기록
        const playsWithin7 = playsFromOpen.filter(
          (t) => t.getTime() < windowEnd.getTime()
        );
    ...
    const weekStatesWithLock = intermediate.map((w, idx, arr) => {
      ...
      const isLockedToday =
        !openAt || (lockAt !== null && lockAt.getTime() <= today.getTime());
    ...
    const withRoles = weekStatesWithLock.map((w) => {
      ...
      if (!w.openAt || w.openAt.getTime() > today.getTime()) {
        ...
        w.weekNumber === currentWeekNumber + 1
      ) {
        role = "next";
      } else if (
        currentWeekNumber !== null &&
