...
// 공통 오디오 소스 (세션 무관)
// ─────────────────────────────────────────────
const NARRATION_SRC = "/assets/sea-of-understanding/audio/narration_understanding.mp3";
const SINGING_BOWL_SRC = "/assets/sea-of-understanding/audio/singing_bowl.mp3";
...
export default function UnderstandingPage() {
  ...
  useEffect(() => {
    ...
    if (narrationRef.current) {
      narrationRef.current.volume = isMuted || isNarrationMuted ? 0 : volume;
    }
    if (singingBowlRef.current) {
      singingBowlRef.current.volume = isMuted || isSingingBowlMuted ? 0 : volume;
    }
  ...
  useEffect(() => {
    ...
    if (isNarrationMuted || narrationHasEnded) {
      ...
      bowlIntervalRef.current = setInterval(playSingingBowlChime, intervalMs);
    }
        ...
        <div className={styles.contentLayer}>
          ...
          <div className={styles.playerArea}>
            ...
            <div className={styles.extraControls}>
              <button
                className={`${styles.audioToggleBtn} ${
                  isNarrationMuted ? styles.audioToggleOff : styles.audioToggleOn
                }`}
                onClick={toggleNarration}
                aria-label={isNarrationMuted ? "내레이션 켜기" : "내레이션 끄기"}
              >
                {isNarrationMuted ? (
              ...
              <div className={styles.bowlGroup}>
                <button
                  className={`${styles.audioToggleBtn} ${
                    isSingingBowlMuted ? styles.audioToggleOff : styles.audioToggleOn
                  }`}
                  onClick={toggleSingingBowl}
                  aria-label={isSingingBowlMuted ? "싱잉볼 켜기" : "싱잉볼 끄기"}
                >
                  {isSingingBowlMuted ? (
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
                        ...
                        {BOWL_INTERVAL_OPTIONS.map((min) => (
                          <button
                            ...
                            className={`${styles.intervalOption} ${
                              min === bowlInterval ? styles.intervalOptionActive : ""
                            }`}
            ...
            <div className={styles.controls}>
              <button className={styles.controlBtn} onClick={handleStop} aria-label="정지">
