...
import { ensureValidToken } from "@/auth/token-manager";
...
async function loadSleepLogFromAPI(
  dateKey: string
): Promise<(SleepLog & { checkedHabits: string[] }) | null> {
  try {
    const tokens = await ensureValidToken();
    const token = tokens?.idToken ?? null;
...
function TimeSelect({
  ...
  const [hour, minute] = value ? value.split(":").map((v) => parseInt(v, 10)) : [-1, -1];
...
export default function PSQITest({
  onSubmit,
  initialResult,
  weekNumber = 1,
  adminHabits = [],
  ...
  const updateAnswer = useCallback(<K extends keyof PSQIAnswers>(key: K, value: PSQIAnswers[K]) => {
      ...
      <div className={styles.container}>
        ...
        >
          <div className={styles.miniScoreCircle} style={{ borderColor: quality.color }}>
            <span className={styles.miniScoreNum} style={{ color: quality.color }}>
              {toScore10(result.total)}
              <small style={{ fontSize: "0.55em", opacity: 0.7 }}>/10</small>
            </span>
          </div>
          <div className={styles.miniScoreInfo}>
            ...
            <div className={styles.miniScoreGradeRow}>
              <span className={styles.miniScoreGrade} style={{ background: quality.color }}>
                {quality.grade}
              </span>
          ...
          <>
            <div className={styles.trackerCard}>
                ...
                {isPastDate &&
                  (isEditMode ? (
                    ...
                    >
                      <svg width="14" height="14" viewBox="0 0 16 16" fill="none">
                        <path
                          d="M11.5 1.5a2.121 2.121 0 113 3L5 14l-4 1 1-4 9.5-9.5z"
                          stroke="#6b7280"
                          strokeWidth="1.5"
                          strokeLinecap="round"
                          strokeLinejoin="round"
              ...
              <div className={styles.dateNav}>
                ...
                <span className={styles.dateNavLabel}>{formatDateLabel(selectedDate)}</span>
                <button
                ...
                <div className={styles.sleepField}>
                  ...
                  {isDateLocked ? (
                    <span className={styles.lockedValue}>
                      {sleepLog.wakeCount >= 0 ? `${sleepLog.wakeCount}회` : "—"}
                    </span>
                  ) : (
                    <div className={styles.wakeCountRow}>
                      <button
                        ...
                        onClick={() =>
                          updateSleepLog({ wakeCount: Math.max(0, sleepLog.wakeCount - 1) })
                        }
                        disabled={sleepLog.wakeCount < 0}
                      >
                        −
                      </button>
                      <span className={styles.wakeCountNum}>
                        {sleepLog.wakeCount >= 0 ? `${sleepLog.wakeCount}회` : "—"}
                      </span>
                      <button
                        className={styles.wakeCountBtn}
                        onClick={() =>
                          updateSleepLog({
                            wakeCount: sleepLog.wakeCount < 0 ? 0 : sleepLog.wakeCount + 1,
                          })
                        }
                      >
                        +
                  ...
                  <div className={styles.napToggleRow}>
                    ...
                    {isDateLocked ? (
                      ...
                      <button
                        ...
                        onClick={() =>
                          updateSleepLog({
                            hadNap: !sleepLog.hadNap,
                            ...(!sleepLog.hadNap ? {} : { napStart: "", napEnd: "" }),
                          })
                        }
                        aria-label="낮잠 여부 토글"
                      >
              ...
              <div className={styles.sleepMoodSection}>
                ...
                {sleepLog.sleepMood && SLEEP_MOOD_MESSAGES[sleepLog.sleepMood] && (
                  <div className={styles.sleepMoodMessage}>
                    {SLEEP_MOOD_MESSAGES[sleepLog.sleepMood](userName || "회원")
                      .split("\n")
                      .map((line, i) => (
                        <p key={i} className={i === 0 ? styles.sleepMoodMessageFirst : undefined}>
                          {line}
                        </p>
                      ))}
                  </div>
              ...
              {weekNumber >= 5 && (
                <div className={styles.sleepNoteSection}>
                  <label className={styles.sleepNoteLabel}>
                    오늘 수면에 영향을 준 한 가지가 있다면?
                  </label>
                  {isDateLocked ? (
              ...
              <div className={styles.habitList}>
                ...
                {adminHabits.map((item) => {
                  ...
                  return (
                    <label
                      key={`admin-${item}`}
                      className={`${styles.habitItem} ${styles.habitItemAdmin} ${isDateLocked ? styles.habitItemLocked : ""}`}
                    >
                      <input
                        type="checkbox"
                        ...
                        checked={checked}
                        onChange={() => {
                          if (!isDateLocked) toggleHabitCheck(item);
                        }}
                        disabled={isDateLocked}
                      />
                      <span
                        className={`${styles.habitCheckMark} ${checked ? styles.habitChecked : ""}`}
                      >
                        {checked ? "✓" : ""}
                      </span>
                      <span
                        className={`${styles.habitText} ${checked ? styles.habitTextChecked : ""}`}
                      >
                        {item}
                      </span>
                    </label>
                ...
                {/* 사용자 커스텀 습관 (하단, 삭제 가능) */}
                {habitItems
                  .filter((item) => !adminHabits.includes(item))
                  .map((item) => {
                    const checked = sleepLog.checkedHabits.includes(item);
                    return (
                      <label
                        key={`user-${item}`}
                        className={`${styles.habitItem} ${isDateLocked ? styles.habitItemLocked : ""}`}
                      >
                        <input
                          type="checkbox"
                          className={styles.habitCheckbox}
              ...
              {!isDateLocked && showAddInput && (
                <div className={styles.addHabitRow}>
                  <input
                    ...
                    autoFocus
                  />
                  <button className={styles.addHabitConfirm} onClick={addHabitItem}>
                    추가
                  </button>
                  <button
                    className={styles.addHabitCancel}
                    onClick={() => {
                      setShowAddInput(false);
                      setNewHabitText("");
                    }}
      ...
      <div className={styles.container}>
        ...
        <div className={styles.resultCard}>
          ...
          <div className={styles.percentCircle} style={{ borderColor: quality.color }}>
            ...
            <span className={styles.percentUnit} style={{ color: quality.color }}>
              /10
            </span>
          </div>
          ...
          <details className={styles.detailToggle}>
            ...
            <div className={styles.detailContent}>
              <div className={styles.rawScoreRow}>
                ...
                <span className={styles.rawScoreValue}>
                  {toScore10(result.total)} <span className={styles.rawScoreMax}>/ 10</span>
                </span>
              </div>
        ...
        <div className={styles.symptomSection}>
          ...
          <p className={styles.symptomConnect}>
            따로따로인 것 같지만,
            <br />이 증상들은 <strong>하나로 연결</strong>되어 있습니다.
          </p>
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        ...
        <p className={styles.subtitle}>Pittsburgh Sleep Quality Index (PSQI)</p>
        <p className={styles.instruction}>지난 한 달 동안의 평소 수면 습관에 대해 답변해 주세요.</p>
      </div>
      ...
      <div className={styles.section}>
        ...
        <div className={styles.questionGroup}>
          <label className={styles.questionLabel}>3. 보통 아침에 몇 시에 일어났습니까?</label>
          <TimeSelect
            value={answers.q3}
      ...
      {/* Q9 */}
      <div className={styles.section}>
        <h3 className={styles.sectionTitle}>9. 전반적인 수면의 질을 어떻게 평가하시나요?</h3>
        <div className={styles.radioGroup}>
          {[
            { value: 0, label: "매우 좋은" },
            { value: 1, label: "꽤 좋은" },
      ...
      <div className={styles.submitSection}>
        <button
          className={`${styles.submitBtn} ${!isComplete() ? styles.submitBtnDisabled : ""}`}
          onClick={handleSubmit}
          disabled={!isComplete() || saving}
        >
          {saving ? "저장 중..." : "결과 확인하기"}
        </button>
        {!isComplete() && <p className={styles.submitHint}>모든 문항에 답변해 주세요.</p>}
        {standalone && onSkip && (
...
function FrequencyQuestion({
  ...
  return (
    <div className={styles.freqQuestion}>
      {!hideLabel && label && <span className={styles.freqLabel}>{label}</span>}
      <div className={styles.radioGroup}>
