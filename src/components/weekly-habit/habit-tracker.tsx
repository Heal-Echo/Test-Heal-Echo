...
export default function HabitTracker({ habits, weekNumber, program }: Props) {
  ...
  const loadRecords = useCallback(async () => {
    try {
      const res = await fetch(`/api/user/habit-tracking/${weekNumber}?program=${program}`);
      if (!res.ok) return;
  ...
  const availableCells = todayIdx >= 0 ? habits.length * (todayIdx + 1) : totalCells;
  const completionRate = availableCells > 0 ? Math.round((checkedCount / availableCells) * 100) : 0;
    ...
    <div className={styles.wrapper}>
      ...
      <div className={styles.dayHeaders}>
        ...
        {weekDates.map((date, idx) => (
          <span
            key={date}
            className={`${styles.dayHeaderLabel} ${date === today ? styles.dayHeaderToday : ""}`}
          >
            {DAY_LABELS[idx]}
          </span>
        ))}
      ...
      {habits.map((habit, habitIdx) => (
        <div key={habitIdx} className={styles.habitRow}>
          ...
          {weekDates.map((date) => {
            const isChecked = checkedMap[date]?.[String(habitIdx)] ?? false;
