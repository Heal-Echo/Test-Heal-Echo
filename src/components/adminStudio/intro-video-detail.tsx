...
import { updateVideo, initiateUpload, completeUpload } from "@/api/admin-client";
...
export default function IntroVideoDetail({
  ...
  const thumbnail = video.thumbnailKey ? makeThumbnailUrl(video.thumbnailKey) : null;
  ...
  async function handleReplaceVideo() {
    ...
    try {
      ...
      for (let partNumber = 1; partNumber <= totalParts; partNumber++) {
        ...
        const uploadUrl =
          partRes.data?.uploadUrl ?? partRes.data?.url ?? partRes.data?.presignedUrl;
  ...
  return (
    <div className="intro-modal-overlay">
      <div className="intro-modal">
        ...
        {thumbnail ? (
          <img src={thumbnail} alt="thumbnail" className="intro-thumbnail-preview" />
        ) : (
        ...
        {/* ----------------------------
            메타 저장 — 유지
           ---------------------------- */}
        <button className="intro-upload-btn mt-6" disabled={saving || replacing} onClick={saveMeta}>
          {saving ? "저장 중..." : "저장"}
        </button>
