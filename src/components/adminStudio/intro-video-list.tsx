...
export default function IntroVideoList({ onSelect }: { onSelect: (video: Video) => void }) {
  ...
  return (
    <div className="mt-10">
      ...
      {!video && <p className="text-slate-500 text-sm">업로드된 Introduction 영상이 없습니다.</p>}
        ...
        <table className="w-full border-collapse intro-table-fixed text-sm">
          ...
          <tbody>
            <tr className="border-b hover:bg-slate-50 transition">
              ...
              <td className="p-3">
                <div className="flex items-center gap-3">
                  <div className="video-thumb-wrapper">
                    <video src={makeVideoUrl(video.key)} className="video-thumb" muted />
                  </div>
              ...
              {/* 업로드 날짜 */}
              <td className="p-3 text-slate-600">{video.createdAt?.slice(0, 10)}</td>
