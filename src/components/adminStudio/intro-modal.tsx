  ...
  return (
    <div className="intro-modal-overlay">
      <div className="intro-modal">
        ...
        <input className="intro-input" value={title} onChange={(e) => setTitle(e.target.value)} />
        ...
        {thumbPreview && <img src={thumbPreview} className="intro-thumbnail-preview" />}

        {/* 업로드 버튼 */}
        <button className="intro-upload-btn" disabled={busy} onClick={handleUpload}>
          {busy ? "업로드 중..." : "업로드"}
        </button>

        {/* 진행률 */}
        {progress > 0 && <p className="intro-progress">진행률: {progress}%</p>}
      </div>
    </div>
  );
}
