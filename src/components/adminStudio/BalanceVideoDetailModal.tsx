...
import BalanceThumbnailModal from "@/components/adminStudio/BalanceThumbnailModal";
...
export default function BalanceVideoDetailModal({
  ...
  async function mpSignPart(payload: any) {
    const res = await fetch(
      `/api/admin/balance/videos/${program}/${weekNumber}/multipart/part`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
  ...
  async function handleReplaceVideo() {
    ...
    try {
      ...
      for (let partNumber = 1; partNumber <= totalParts; partNumber++) {
        ...
        const uploadUrl =
          partRes?.uploadUrl ?? partRes?.url ?? partRes?.presignedUrl;
