...
export default function SleepHabitStudio({ folder }: Props) {
  ...
  useEffect(() => {
    async function fetchWeekCount() {
      try {
        ...
        const maxWeek = items.reduce((max, item) => Math.max(max, item.weekNumber), 0);
  ...
  function updateRow(id: string, field: "weekNumber" | "habit", value: string | number) {
    setRows((prev) =>
      prev.map((r) =>
        r.id === id ? { ...r, [field]: value, saved: field === "habit" ? false : r.saved } : r
      )
    );
  }
  ...
  async function saveRow(id: string) {
    ...
    try {
      ...
      setRows((prev) => prev.map((r) => (r.id === id ? { ...r, saved: true, saving: false } : r)));
    } catch (err) {
  ...
  async function deleteRow(id: string) {
    ...
    if (target.saved) {
      ...
      const weekHabits = rows
        .filter((r) => r.weekNumber === target.weekNumber && r.id !== id && r.habit.trim())
        .map((r) => r.habit.trim());
  ...
  if (loading) {
    return (
      <div style={{ padding: 40, textAlign: "center", color: "#888" }}>데이터를 불러오는 중...</div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto" }}>
      ...
      >
        주차별 수면 습관을 등록하면, 고객에게 누적으로 표시됩니다. (예: 2주차 고객 = 1주차 + 2주차
        습관)
      </p>
        ...
        <tbody>
          {rows.length === 0 && (
            <tr>
              ...
              >
                등록된 수면 습관이 없습니다. 아래 &quot;+ 새 행 추가&quot; 버튼을 눌러 추가하세요.
              </td>
            </tr>
          )}
            ...
            >
              ...
              <td style={{ padding: "10px 16px", verticalAlign: "middle" }}>
                <div style={{ position: "relative", width: 100 }}>
                  ...
                  <select
                    value={row.weekNumber}
                    onChange={(e) => updateRow(row.id, "weekNumber", Number(e.target.value))}
                    disabled={row.saving}
                    style={{
                      position: "absolute",
                      top: 0,
              ...
              >
                <div style={{ display: "flex", gap: 6, justifyContent: "center" }}>
                  <button
                    ...
                    style={{
                      border: "none",
                      background: row.saving || !row.habit.trim() ? "#e2e8f0" : "#1e293b",
                      color: row.saving || !row.habit.trim() ? "#94a3b8" : "#fff",
                      borderRadius: 6,
                      padding: "6px 14px",
                      fontSize: 12,
                      fontWeight: 600,
                      cursor: row.saving || !row.habit.trim() ? "not-allowed" : "pointer",
                    }}
                  >
                    {row.saving ? "저장 중…" : "저장"}
                  </button>
