...
export default function Studio({ folder }: { folder: string }) {
  ...
  async function mpInitiate(payload: any) {
    const res = await fetch(`/api/admin/balance/videos/${program}/${week}/multipart/initiate`, {
      method: "POST",
  ...
  async function mpSignPart(payload: any) {
    const res = await fetch(`/api/admin/balance/videos/${program}/${week}/multipart/part`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
  ...
  async function mpComplete(payload: any) {
    const res = await fetch(`/api/admin/balance/videos/${program}/${week}/multipart/complete`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    ...
    <div className="space-y-10">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <table className="intro-table-fixed">
          ...
          <tbody>
            {videos.map((v) => (
              <tr key={v.weekNumber} className="border-b hover:bg-slate-50 transition">
                <td className="p-3">{v.weekNumber}주차</td>
                <td className="p-3">{v.title}</td>
                <td className="p-3" style={{ textAlign: "center" }}>
                  {v.key ? (
                    <span title={v.key} style={{ cursor: "help" }}>
                      🎬
                    </span>
                  ) : (
                ...
                <td className="p-3 text-slate-600">{(v as any).createdAt?.slice(0, 10) ?? ""}</td>
                <td className="p-3">
