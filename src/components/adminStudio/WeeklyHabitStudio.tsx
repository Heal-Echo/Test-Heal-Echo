...
export default function WeeklyHabitStudio({ folder }: { folder: string }) {
  ...
  async function mpInitiate(payload: any) {
    const res = await fetch(
      `/api/admin/weekly-habit/${program}/${week}/multipart/initiate`,
      {
        method: "POST",
  ...
  async function mpSignPart(payload: any) {
    const res = await fetch(
      `/api/admin/weekly-habit/${program}/${week}/multipart/part`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
  ...
  async function mpComplete(payload: any) {
    const res = await fetch(
      `/api/admin/weekly-habit/${program}/${week}/multipart/complete`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
          ...
          <div className="intro-modal" style={{ maxWidth: 600, maxHeight: "90vh", overflowY: "auto" }}>
            ...
            {editMode && existingVideoKey && !videoFile && (
              <div style={{
                ...
                <div style={{ marginTop: 6, color: "#4b5563", fontSize: 12, wordBreak: "break-all" }}>
            ...
            {contents.map((c) => (
              ...
              >
                ...
                <td className="p-3" style={{ textAlign: "center" }}>
                  {c.videoKey ? (
                    <span title={c.videoKey} style={{ cursor: "help" }}>🎬</span>
                  ) : (
