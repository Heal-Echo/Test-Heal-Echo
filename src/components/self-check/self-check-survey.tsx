...
export function getSignalIntensity(categories: { percent: number }[]): number {
 ...
 */
async function postSelfCheckToAWS(result: SelfCheckResult, token: string | null): Promise<boolean> {
  try {
...
async function saveSelfCheckResult(result: SelfCheckResult) {
  ...
  if (!ok) {
    ...
    console.warn("[SelfCheck] AWS 저장 실패 — 다음 접속 시 재시도됩니다.");
  }
}
...
export default function SelfCheckSurvey({
  ...
  const [step, setStep] = useState<"intro" | number | "result">(skipIntro ? 0 : "intro");
  ...
  // ── 빈도 변경 ──
  const setFrequency = useCallback((symptomId: string, frequency: FrequencyId) => {
    setAnswers((prev) => prev.map((a) => (a.symptomId === symptomId ? { ...a, frequency } : a)));
  }, []);
    ...
    <div className={styles.main}>
      ...
      {step === "intro" && (
        <div className={styles.introScreen}>
          ...
          <button className={styles.introStartBtn} onClick={() => setStep(0)}>
            시작하기
          </button>
      ...
      {currentCat && (
        <div className={styles.questionScreen} key={currentCat.id}>
              ...
              return (
                <div key={symptom.id}>
                  ...
                  >
                    ...
                    >
                      {isSelected && <span className={styles.symptomCheckMark}>✓</span>}
                    </div>
                      ...
                      {FREQUENCIES.map((f) => (
                        <button
                          ...
                          className={`${styles.frequencyBtn} ${
                            answer?.frequency === f.id ? styles.frequencyBtnActive : ""
                          }`}
          ...
          {(() => {
            ...
            return !hasAny ? (
              <p className={styles.noMatchHint}>해당사항이 없으면 그대로 다음으로 넘어가세요.</p>
            ) : null;
          })()}

          <div className={styles.navButtons}>
            {currentCatIndex > 0 && (
              <button className={styles.prevBtn} onClick={() => setStep(currentCatIndex - 1)}>
                이전
              </button>
            )}
            <button
            ...
            >
              {currentCatIndex < CATEGORIES.length - 1 ? "다음" : "결과 확인하기"}
            </button>
          </div>
        </div>
      )}
            ...
            <div className={styles.resultScreen}>
              ...
              <div className={styles.resultCard}>
                ...
                <div className={styles.scoreCircle} style={{ borderColor: gradeInfo.color }}>
                  ...
                  <span className={styles.scoreUnit} style={{ color: gradeInfo.color }}>
                    %
                  </span>
                </div>
